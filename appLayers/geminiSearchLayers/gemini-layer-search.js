...
【⚠️ 厳守ルール】
1. 抽出対象は、配列の中にある「レイヤー名」のみです。
2. **「カテゴリ名（JSONのキー）」自体をレイヤー名として抽出してはいけません。**
3. 抽出したレイヤーを、ユーザーの要望に対する「関連度（おすすめ順）」が高い順に並べてください。
4. 出力は、必ず以下の形式のJSON配列のみとしてください。説明文やマークダウンは一切禁止です。
