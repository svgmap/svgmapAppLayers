...
var _menutblId;
async function initLoader(targetTabUiId, menuTableId, getSourceFunc, showMapFunc, clearMapFunc){
	await checkIndexTxt();
	initSubLayerList();
...
function dispCsvIndex(csvdata){
	...
	for ( var i = 0 ; i < csvdata.length ; i++ ){
		...
		btn.addEventListener("click",function(fileName){
			return(
				async function(){
...
async function loadByFileName(fileName){
	console.log("loadByFileName:",fileName);
	if ( _int_localFileMode ){
		var content = await localDB.getContent(fileName);
		console.log("content:",content);
		await _int_showMapEmbed(content);
	} else {
