	...
	// Copyright (C) 2013-2024 by Satoru Takagi @ KDDI CORPORATION
	// 
	// License:
	...
	**/
	...
	// 再描画直前に実行されるコールバック関数
		...
		if (level > tileInfo.maxLevel ){
			level = tileInfo.maxLevel;
		} else if ( level < tileInfo.minLevel ){
			level = tileInfo.minLevel;
		}
...
//				ないものなので、消去
	...
	function getURL( tx , ty , lvl ){
		...
		var mapServerURL = tileInfo.baseURL.replaceAll("{z}",lvl);
