...
//    	console.log(hash);
	...
	function getCsv(progressCallBack){
		if ( !csv && window.useQTCT ){
			if ( restoreCsvDataFromZipFile ){ // QTCTrenderer_r2の関数(モジュール化すべき)
				return restoreCsvArrayFromZipFile(progressCallBack); //この場合はpromiseが返ってくる
			} else {
				var csvdat = restoreCsvData(); // QTCTrendererの関数(モジュール化すべき)
	...
	async function restoreCsvArrayFromZipFile(progressCallBack){
		...
		var csvdat = await restoreCsvDataFromZipFile(progressCallBack);
		for ( var i = 0 ; i < csvdat.length ; i++){
			csvdat[i]= csvdat[i].join(",");
		}
		return csvdat;
	...
	function onload(){
		...
		messageDivElm = window.messageDiv;
...
//		console.log("verIE:",verIE);
		...
		var qtctData;
		window.useQTCT=false;
		if ( csv.length > QTCTth){
			window.useQTCT=true;
			qtctData = [];
		}
		
		// console.log(window.useQTCT,csv);
...
//				var wgPos = toWGS( strTxt[2] * 1.0 , strTxt[1] * 1.0);
	...
	async function drawQTCTmap(qtctData, cs){
		...
		await window.buildQTCTdata(qtctData, cs, progressFunc);
		svgMap.refreshScreen();
		messageDivElm.innerText="";
	}
